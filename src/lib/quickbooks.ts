...
export async function buildQuickBooksMcpPrompt(session: QuickBooksSession) {
  ...
  return [
    ...
    "",
    "Tasks:",
    "1. Find the cloned MCP server repository. Confirm it contains package.json and src/index.ts.",
    ...
    "   - Run npm run build.",
    "4. Create or update the repository-local .env file with:",
    ...
    "5. Register the built MCP server globally/user-wide so future Claude Code or Codex chats can use it.",
    ...
    "7. Tell me to start a new Claude Code or Codex chat after registration. Existing chats may not hot-load new MCP tools.",
    "",
